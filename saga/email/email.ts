...
function* getEmailsSaga(action: { type: string, payload: { limit: number, offset: number } }): any {
    try {
        const response = yield call(getEmailsApi, action.payload.limit, action.payload.offset);
