...
import {
    addProject, addProjectFailed,
    addProjectSuccess, deleteProjectItem,
    deleteProjectItemFailed, deleteProjectItemSuccess,
    editProjectItem, editProjectItemFailed,
    editProjectItemSuccess, getProjectItem,
    getProjectItemFailed, getProjectItemSuccess,
    getProjects, getProjectsFailed, getProjectsSuccess
} from "../../store/projects/reducer";
import { ProjectItem } from "../../store/projects/types";

function* getProjectsSaga(action: { type: string, payload: { showHidden: boolean, limit:number, offset:number } }): any {
    try {
        const response = yield call(getProjectsApi,  action.payload.showHidden, action.payload.limit, action.payload.offset);
...
function* getProjectItemSaga(action: { type: string, payload: { id: number } }): any {
    try {
...
function* addProjectSaga(action: { payload: { data: ProjectItem, closeModal: () => void } }) {
    try {
...
function* updateProjectSaga(action: { payload: { data: ProjectItem, closeModal: () => void } }) {
    try {
